...
session_start();
	...
	include ("pagseguro/configuracaopag.php")
?>
<!-- HEAD -->
...
<div id="tc-index">
						...
						if(!isset($_GET['cdus'])){
							...
							if ($result=$mysqli->query($sql)) {
								while ($obj = $result ->fetch_object()) {
									...
									echo "<div class='col-sm-9 text-center'><hr class='hr-perfil'><h3 class='tracking-in-expand'>Serviços Ativos</h3>";
										...
										<div class="card" style="width: 18rem;">
  											<div class="card-body">
   												<h5 class="card-title">Serviço: '.$servicof->nm_servico.'</h5>';
   	    											...
   	    											echo '<a href="nota.php?nota='.$servicof->cd_servico.'"><button class="btn btn-outline-info">Avalie</button></a> ';
   	    											echo ' <a href="chatajax.php?chat='.$servicof->cd_servico.'"><button class="btn btn-outline-info">Chat</button></a><br><br>';
   	    											echo '<button class="btn btn-success" onclick="pagamento()">Pagar</button>';
   	    											echo '<span class="endereco" data-endereco="<?php echo URL; ?>"></span>';

   													}else{
   														echo '<p class="card-text">Parabéns, escolheram seu orçamento. Quando realizar o serviço avalie o usuario contratante.</p>';
   														echo '<a href="nota.php?notado='.$servicof->cd_servico.'"><button class="btn btn-outline-info">Avalie</button></a> ';
   														echo ' <a href="chatajax.php?chat='.$servicof->cd_servico.'"><button class="btn btn-outline-info">Chat</button></a>';
   													}
    											echo '</div>
										</div>
										</a>';	
	...
	printf($mysqli->error);
									...
									if ($obj->st_admin == 0) {
										echo "<div class='col-sm-12 text-center'><hr class='hr-perfil'><h3 class='tracking-in-expand'>Galeria</h3>";
										echo "<button class='btn btn-success' data-toggle='modal' data-target='#galeriaModal'>Adicione Fotos</button></div><br>";
    ...
    <div class="modal-dialog">
      <div class="modal-content bg-custom">
        ...
        <div class="modal-footer">
          <button style="text-transform: capitalize;" type="button" class="btn btn-warning" data-dismiss="modal"><i class='fas fa-angle-left'></i> Voltar</button>
        </div>
      </div>
    </div>
</div>
      ...
      <div class="modal-content bg-custom">
        ...
        <div class="modal-body">
        	<center>
				...
				<input type="file" name="fotos[]" required class="btn btn-outline-danger editfoto" id="upload" multiple="multiple"></label>
				<img id="img_edit" style="width: 300px;" class="edit_foto">
				<br>
